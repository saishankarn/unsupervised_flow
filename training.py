...
The program contains the training function
...
"""
sourceImgsList = '/home/sanjeevs/unsupervised_flow/source_names.txt'
targetImgsList = '/home/sanjeevs/unsupervised_flow/target_names.txt'

logDirectory     = '/home/sanjeevs/unsupervised_flow/log_directory'
checkpointPath   = '/home/sanjeevs/unsupervised_flow/log_directory/swaayatt/model-7'
outputPath_image = '/home/sanjeevs/unsupervised_flow/results'
outputPath_flow  = '/home/sanjeevs/unsupervised_flow/results1'
targetLanePath   = '/home/sanjeevs/unsupervised_flow/results2'
...
def generateTargetLanes(params, sourceLoc, targetLoc, sourceLaneLoc, source_target_flow, target_source_flow):

	sess = sessionSetup()

	params.batchSize = 1

	sourceLane   = tf.placeholder(shape = [params.batchSize, params.nRows, params.nCols, 3], dtype = tf.float32, name = "source_lane_image")
	s_t_flow     = tf.placeholder(shape = [params.batchSize, params.nRows, params.nCols, 2], dtype = tf.float32, name = "source_target_flow")

	s_t_x_flow = s_t_flow[:, :, :, 0]
	s_t_y_flow = s_t_flow[:, :, :, 1]

	targetLane = generate_image(sourceLane, s_t_x_flow, s_t_y_flow)
	#targetLane = flowWarp(sourceLane, s_t_flow)

	init_op = tf.global_variables_initializer()
	
	sess.run(init_op)

	sourceLaneLabel = cv2.resize(cv2.imread(sourceLaneLoc), (params.nCols, params.nRows))

	sourceLaneLabel = np.expand_dims(sourceLaneLabel, axis = 0)

	#source_target_flow = cv2.resize(source_target_flow, (params.nCols, params.nRows)) 

	#sourceLane = cv2.resize(cv2.imread(sourceLaneLoc), (params.nCols, params.nRows))

	targetLaneLabel = sess.run((targetLane), feed_dict = {sourceLane : sourceLaneLabel, s_t_flow : source_target_flow})

	return targetLaneLabel

def generateLabeledImage(params, targetLane, targetLoc, sourceLaneLoc):

	targetLane = targetLane

	targetImage = cv2.resize(cv2.imread(targetLoc), (params.nCols, params.nRows))

	targetImage = targetImage + targetLane 
...
def main():
	...
	"""
	sourceLoc = '/home/sanjeevs/unsupervised_flow/test/'
	targetLoc = '/home/sanjeevs/unsupervised_flow/test/'

	sourceLaneLoc = '/home/sanjeevs/toSai/s_imgsOutL/img_20.png'
	#sourceLaneLoc = '/home/sanjeevs/unsupervised_flow/results2/targetLane.png'
	...
	for i in range(10):
		...
		sourceImgLoc = sourceLoc + str(frame).zfill(6) + '.png'

		frame = frame + 1 

		targetImgLoc = targetLoc + str(frame).zfill(6) + '.png'
	
		T_S_Flow, S_T_Flow = test(params, sourceImgLoc, targetImgLoc, checkpointPath)

		targetLane = generateTargetLanes(params, sourceImgLoc, targetImgLoc, sourceLaneLoc, S_T_Flow[0], T_S_Flow[0])

		labeled_target = generateLabeledImage(params, targetLane[0], targetImgLoc, sourceLaneLoc)

		lane = targetLane[0] * 255

		laneImagePath = targetLanePath + '/' + 'targetLane' + '_' + str(i + 1) + '.png'

		cv2.imwrite(laneImagePath, lane)

		sourceLaneLoc = laneImagePath

		labeledImgPath = targetLanePath + '/' + 'augLane_' + str(i + 1) + '.png'

		cv2.imwrite(labeledImgPath, labeled_target)
	"""
